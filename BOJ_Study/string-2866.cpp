/*
해당 문제는 이분 탐색을 이용하여 주어진 문자열 행렬에서 중복이 발생하지 않는 최대 행을 찾는 문제이다
...
int main() 
{
    ...
    for (int i = 1; i <= r; i++) 
    {
        cin >> arr[i]; // 각각의 행에 대한 문자열을 입력 받는다. 예)dobarz, adatak
    }
    ...
    while (start <= end) 
    {
        ...
        /* 
        set은 C++ 표준 라이브러리의 컨테이너 중 하나로, 중복된 원소를 가질 수 없는 컨테이너로,
        Key라 불리는 원소들의 집합으로 이루어진 컨테이너이다 (원소 = key)
        set의 각 원소는 고유해야 하며, 이는 자동으로 정렬 (default : less(오름차순)) 되어 저장된다
        */ 
        set<string> s; // set은 주어진 문자열 행렬에서 각 열을 기준으로 생성된 문자열들이 중복되는지 여부를 빠르게 판단하는데 사용
        bool check = true;
        ...
        for (int i = 0; i < c; i++) 
        {
            ...
            else 
            {
                ...
                break;
                /* 
                만약 현재 생성된 문자열 tmp가 이미 set에 있다면 중복된 문자열이 있는 것으로 판단하고,
                체크 변수(check)를 false로 설정하고 for 반복문을 종료하고 나온다 
                */
            }
        }
        ...
        else // 'check' 변수가 false인 경우 다음 탐색 범위를 줄인다
        {
